/*document.addEventListener("DOMContentLoaded", function () {
    ...
    // Récupérer l'ID du film et la date du jour
    const movieId = screeningsData.getAttribute("data-movie-id");
    const todayDate = screeningsData.getAttribute("data-today");
    ...
    function loadScreenings(movieId, date) {
        fetch(`index.php?action=getScreenings&movie_id=${movieId}&date=${date}`)
            .then((response) => response.json())  // Directement récupérer la réponse JSON
            .then((data) => {
    ...
    // Charger les séances d'aujourd'hui par défaut
    loadScreenings(movieId, todayDate);

    // Ajouter un événement sur chaque bouton pour charger les séances du jour sélectionné
    dayButtons.forEach((button) => {
        button.addEventListener("click", function () {
            const selectedDate = button.getAttribute("data-day");
            //console.log("totoooooooooo " + todayDate);
            // Charger les séances pour la date sélectionnée
            loadScreenings(movieId, selectedDate);
